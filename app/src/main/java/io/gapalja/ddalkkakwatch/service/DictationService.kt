...
/**
 * BLE HID 연결을 백그라운드에서 유지하는 Foreground Service.
 *
 * 목적: Knock-Knock(손목 두 번) → QuickDictateActivity launch 시,
 * BLE가 cold start면 연결에 1~2초 걸려 첫 트리거가 씹힘.
 * 이 서비스가 미리 연결을 warm하게 유지 → 손목 두드리자마자 즉시 전송.
 *
...
class DictationService : Service() {
    ...
    override fun onCreate() {
        ...
        startInForeground()
        ble.register()
        Log.i(TAG, "onCreate — BLE keep-alive 시작")
    }
    ...
    private fun createChannel() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            ).apply { description = "워치 ↔ Mac BLE 연결을 백그라운드에서 유지합니다." }
